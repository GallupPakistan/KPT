# views/exports.py  —  Export Analysis  (13 charts)
...
def _sec(title):
    ...
    <div style="font-size:12px;font-weight:700;color:#111111;letter-spacing:2px;
        text-transform:uppercase;border-bottom:1px solid #cc333333;
...
def render(df):
    ...
    exp = df[df['Category'] == 'Exports / Million Tons'].copy()
    if exp.empty:
        st.warning("⚠️ No export data for the selected range.")
        return

    if not pd.api.types.is_datetime64_any_dtype(exp['Period Start']):
        exp['Period Start'] = pd.to_datetime(exp['Period Start'], format='mixed', errors='coerce')

    exp['Year']       = exp['Period Start'].dt.year
    exp['MonthNum']   = exp['Period Start'].dt.month
    exp['MonthName']  = exp['Period Start'].dt.strftime('%b')
    exp['MonthLabel'] = exp['Period Start'].dt.strftime('%b-%Y')

    bulk    = exp[exp['Cargo'] == 'Bulk Cargo'].sort_values('Period Start')
    general = exp[exp['Cargo'] == 'General Cargo'].sort_values('Period Start')
    dry     = exp[exp['Cargo'] == 'Total Dry Cargo'].sort_values('Period Start')
    liquid  = exp[exp['Cargo'] == 'Total Liquid Cargo'].sort_values('Period Start')
    total   = exp[exp['Cargo'] == 'Total Cargo'].sort_values('Period Start')

    # ── 1. Total Trend + Rolling Avg ────────────────────────────────────────────
    _sec("📈 Total Export Volume — Monthly Trend")
    ...
    fig1.add_trace(go.Scatter(
        ...
        mode='lines+markers', name='Total Exports',
    ...
    fig1.update_layout(**PLOT_BG, height=340,
                       title=dict(text='Monthly Total Exports with 6-Month Rolling Average',
    ...
    with c1:
        ...
        fig3a.update_layout(**PLOT_BG, height=300,
                            title=dict(text='Total Dry Cargo Exports',
                                       font=dict(color='#444444', size=12)))
    ...
    for col_name, color, fill_color in [
        ('Liquid',  C['liquid'],  'rgba(194,24,91,0.65)'),
        ('General', C['general'], 'rgba(91,77,196,0.65)'),
    ...
    fig4.update_layout(**PLOT_BG, height=320,
                       title=dict(text='Export Composition: Bulk + General + Liquid (Stacked Area)',
                                  font=dict(color='#444444', size=12)))
    _ax(fig4, ytitle='Million Tons (Stacked)')
    st.plotly_chart(fig4, use_container_width=True)

    # ── 5. Year-wise Annual Bars (2 cols) ───────────────────────────────────────
    _sec("📅 Year-wise Annual Export Summary")
    ...
    with c3:
        ...
        fig5a.update_layout(**PLOT_BG, height=320,
                            title=dict(text='Annual Total Exports by Year',
                                       font=dict(color='#444444', size=12)))
        _ax(fig5a, ytitle='Million Tons', tickmode='linear', dtick=1)
        st.plotly_chart(fig5a, use_container_width=True)

    with c4:
        grp = exp[exp['Cargo'].isin(['Bulk Cargo','General Cargo','Total Liquid Cargo'])]
    ...
    _sec("🌡️ Export Intensity Heatmap — Month × Year")
    ...
    fig6 = go.Figure(go.Heatmap(
        ...
        y=list(pivot.index),
        colorscale=[[0,'#fef0f0'],[0.3,'#f5a0a0'],[0.6,'#e05555'],[1,'#aa1111']],
        showscale=True,
    ...
    fig6.update_layout(**PLOT_BG, height=380,
                       title=dict(text='Monthly Export Intensity — Brighter = Higher Volume',
    ...
    with c5:
        ...
        fig7a.update_layout(**PLOT_BG, height=300,
                            title=dict(text='Bulk Cargo Exports — Monthly Bar',
                                       font=dict(color='#444444', size=12)))
    ...
    with c6:
        ...
        fig7b.update_layout(**PLOT_BG, height=300,
                            title=dict(text='General Cargo Exports — Monthly Bar',
                                       font=dict(color='#444444', size=12)))
    ...
    share_df = exp[exp['Cargo'].isin(['Bulk Cargo','General Cargo','Total Liquid Cargo'])]
    ...
    with c7:
        ...
        fig8a.update_layout(**PLOT_BG, height=300, showlegend=False,
                            title=dict(text='Export Share by Cargo Type',
                                       font=dict(color='#444444', size=12)))
    ...
    _sec("📆 Seasonal Pattern — Average Exports by Month")
    ...
    fig9 = go.Figure(go.Bar(
        x=seas['MonthName'],
        y=seas['AvgFigures'],
        marker_color=[C['total'] if v >= overall_avg else '#f5c0c0'
                      for v in seas['AvgFigures']],
    ...
    fig9.update_layout(**PLOT_BG, height=300,
                       title=dict(text='Average Monthly Export Volume — Seasonal Pattern',
                                  font=dict(color='#444444', size=12)))
    ...
    fig10.update_layout(**PLOT_BG, height=300,
                        title=dict(text='Year-over-Year Export Growth Rate (%)',
                                   font=dict(color='#444444', size=12)))
    _ax(fig10, ytitle='Growth %', tickmode='linear', dtick=1)
    st.plotly_chart(fig10, use_container_width=True)

    # ── 11. Month-over-Month Change ─────────────────────────────────────────────
    _sec("🔄 Month-over-Month Change — Total Exports")
    ...
    fig11.update_layout(**PLOT_BG, height=280,
                        title=dict(text='Month-over-Month Export Change (Blue = Growth, Red = Decline)',
                                   font=dict(color='#444444', size=12)))
    ...
    sb = exp[exp['Cargo'].isin(['Bulk Cargo','General Cargo','Total Liquid Cargo'])]
    ...
    fig12.update_layout(**PLOT_BG, height=320, barmode='stack',
                        title=dict(text='Yearly Export Composition — Stacked (Bulk + General + Liquid)',
                                   font=dict(color='#444444', size=12)))
    _ax(fig12, ytitle='Million Tons', tickmode='linear', dtick=1)
    st.plotly_chart(fig12, use_container_width=True)

    # ── 13. Top 10 Highest & Lowest Export Months (2 cols) ──────────────────────
    _sec("🏆 Top 10 Highest & Lowest Export Months")
    ...
    with c9:
        ...
        fig13a.update_layout(**PLOT_BG, height=380,
                             title=dict(text='Top 10 Highest Export Months',
                                        font=dict(color='#444444', size=12)))
    ...
    with c10:
        ...
        fig13b.update_layout(**PLOT_BG, height=380,
                             title=dict(text='Top 10 Lowest Export Months',
                                        font=dict(color='#444444', size=12)))
    ...
    <div style="text-align:center;color:#888888;font-size:10px;letter-spacing:2px;
        ...
        EXPORT ANALYSIS · KARACHI PORT TRUST · ALL FIGURES IN MILLION TONS
